...
package app.kuwas.android.bridge.leagues;
...
import app.kuwas.android.bridge.util.Util;
import io.brookmg.soccerethiopiaapi.access.SoccerEthiopiaApi;
import io.brookmg.soccerethiopiaapi.network.StandingFetch;

public class SoccerEthiopiaConnector implements Bridge {

    SoccerEthiopiaApi mainApi;

    public SoccerEthiopiaConnector(Context appContext, boolean shouldCache) {
        mainApi = new SoccerEthiopiaApi(appContext, shouldCache);
    }

    @Override
    public void getLatestTeamRanking(OnItemProcessed<ArrayList<RankItem>> processed, final OnError error) {
        mainApi.getLatestTeamRanking(
                ranking -> {
                    ArrayList<RankItem> items = new ArrayList<>();
                    Util.copyFieldsInClassToAnother(ranking , items);
                    processed.onFinish(items);
                },
                error::onError
        );
    }
